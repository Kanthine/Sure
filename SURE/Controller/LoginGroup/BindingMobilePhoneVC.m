...
@interface BindingMobilePhoneVC ()
...
@implementation BindingMobilePhoneVC
...
- (void)rightBarButtonClick
{
    ...
    {
        BindedMobilePhoneVC *bindedVC = [[BindedMobilePhoneVC alloc]init];
        bindedVC.phoneStr = _phoneNumberString;
        bindedVC.headerURLStr = self.headerURLStr;
        bindedVC.userIDStr = self.userIDStr;
        bindedVC.verificationCodeString = _verificationCodeString;
        [self.navigationController pushViewController:bindedVC animated:YES];
    }
    else
    {
        //提示手机号码有问题
...
- (IBAction)getVerificationCode:(UIButton *)sender
{
    ...
    if (_verificationCodeButton.enabled)
    {
        ...
        NSDictionary *dict = @{@"mobile_phone":_phoneNumberTf.text,@"user_id":self.userIDStr};
