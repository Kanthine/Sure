...
- (UIView *)topView
{
    if (_topView == nil)
    {
        ...
        flashModelButton.frame = CGRectMake(0, 0, 60, 44);
        UIImageView *flashImageView = [[UIImageView alloc]initWithFrame:CGRectMake(10, 10, 25, 25)];
        flashImageView.contentMode = UIViewContentModeScaleAspectFit;
        flashImageView.image = [UIImage imageNamed:@"cameraFlash_Off"];
        flashImageView.tag = 23;
        [flashModelButton addSubview:flashImageView];
        [flashModelButton addTarget:self action:@selector(flashModelButtonClick:) forControlEvents:UIControlEventTouchUpInside];
        [_topView addSubview:flashModelButton];

        
        
        UIButton *camearPostionButton = [UIButton buttonWithType:UIButtonTypeCustom];
        camearPostionButton.frame = CGRectMake(ScreenWidth - 60, 0, 60, 44);
        UIImageView *postioImageView = [[UIImageView alloc]initWithFrame:CGRectMake(25, 10, 25, 25)];
        postioImageView.contentMode = UIViewContentModeScaleAspectFit;
        postioImageView.image = [UIImage imageNamed:@"camearSwitch"];
        [camearPostionButton addSubview:postioImageView];
        [camearPostionButton addTarget:self action:@selector(camearPostionButtonClick:) forControlEvents:UIControlEventTouchUpInside];
        [_topView addSubview:camearPostionButton];
...
- (UIView *)bottomView
{
    if (_bottomView == nil)
    {
        ...
        cancelButton.frame = CGRectMake(10, 15, 60, 60);
        ...
        photoButton.frame = CGRectMake(ScreenWidth * 1/2.0 - 30,15, 60, 60);
        [photoButton setImage:[UIImage imageNamed:@"camera_Photo"] forState: UIControlStateNormal];
        [photoButton addTarget:self action:@selector(shutterCameraClick) forControlEvents:UIControlEventTouchUpInside];
...
- (UIView *)streamesView
{
    ...
    {
        _streamesView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight)];
    ...
    [self.ImageOutPut captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error)
     {
         ...
         PhotoCropViewController *cropVC = [[PhotoCropViewController alloc]initWithImage:image cropFrame:CGRectMake(0, (ScreenHeight - 64 - self.cropSize.height) / 2.0, self.cropSize.width, self.cropSize.height) limitScaleRatio:3];
