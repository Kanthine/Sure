...
#define BackImageHeight (250.0 / 375.0 * ScreenWidth)
...
@property (nonatomic, strong) UIImageView *showImgView;//界面展示image
@property (nonatomic, strong) UIView *overlayView;//50%黑色覆盖图层
@property (nonatomic, strong) UIView *ratioView;//裁剪区域

@property (nonatomic, assign) CGRect cropFrame;//裁剪尺寸
@property (nonatomic, assign) CGRect oldFrame;//原始图片在界面的尺寸坐标
@property (nonatomic, assign) CGRect largeFrame;//能最大放大的坐标
@property (nonatomic, assign) CGFloat limitRatio;//最大倍数
...
- (void)initView
{
    ...
    UIImageView *coverImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"photoCover"]];
    coverImageView.layer.borderWidth = .7f;
    coverImageView.frame = self.ratioView.bounds;
    coverImageView.layer.borderColor = [UIColor whiteColor].CGColor;
    [self.ratioView addSubview:coverImageView];
...
- (void)confirm:(id)sender
{
    ...
    if (self.cropFrame.size.height == ScreenWidth && self.cropFrame.size.width == ScreenWidth)
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"myCropHeaderImageSuccess" object:nil userInfo:@{@"image":image}];
    }
    else
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"myCropImageSuccess" object:nil userInfo:@{@"image":image}];
...
-(UIImage *)getSubImage
{
    ...
    NSLog(@"myImageRect ====== %@",[NSValue valueWithCGRect:myImageRect]);
