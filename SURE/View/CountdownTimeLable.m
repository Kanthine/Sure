...
@implementation CountdownTimeLable

-(id)init{
    self = [super init];
    if (self) {
        _timeLabel = self;
        _timerType = CountTimerLabelTypeStopWatch;
    }
    return self;
}
...
-(void)start{
    ...
    if(_timer == nil){
        ...
        if(startCountDate == nil){
            ...
            if (_timerType == CountTimerLabelTypeStopWatch && timeUserValue > 0) {
...
-(void)updateLabel:(NSTimer*)timer{
    ...
    if(_timerType == CountTimerLabelTypeStopWatch){
